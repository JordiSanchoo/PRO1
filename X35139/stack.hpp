...
public:
  ...
  void removeFirstOccurrence(T value) {
  	if (_size != 0){
  		Item *pact = ptopitem;
  		Item *pant = NULL;
  		while (pact != NULL and pact->value != value){
  			pant = pact;
  			pact = pact->next;
  		}
  		if (pact != NULL){ // hem trobat value
  			if (pant != NULL){// value no és el primer element de la pila
  				pant->next = pact->next;
  				delete pact;
  				_size--;
  			}
  			else{ // value és el primer element de la pila
  				ptopitem = ptopitem->next;
  				delete pact;
  				_size--;
  			}	
