...
public:
  ...
  void swapFirstLast() {
	if (_size <= 1) return;
	if (_size > 2){
  		Item *ptrprimer = iteminf.next;
  		Item *ptrultim = itemsup.prev;
  		Item *ptrsegon = ptrprimer->next;
  		Item *ptrpenultim = ptrultim->prev;

		ptrultim->prev = &iteminf;
  		ptrultim->next = ptrsegon;
  		iteminf.next = ptrultim;
  		ptrsegon->prev = ptrultim;
  		
  		itemsup.prev = ptrprimer;
  		ptrprimer->next = &itemsup;
  		ptrprimer->prev = ptrpenultim;
  		ptrpenultim->next = ptrprimer;
  	}
	if (_size == 2){
		Item *ptrprimer = iteminf.next;
  		Item *ptrultim = itemsup.prev;
		ptrprimer->next = &itemsup;
        ptrprimer->prev = ptrultim;
        itemsup.prev = ptrprimer;
        ptrultim->next = ptrprimer;
        ptrultim->prev = &iteminf;
        iteminf.next = ptrultim;
	}
  }
