...
public:
  ...
  void operator+=(const Queue<T> &q) {
	  if (q.first != NULL){ // Si q no està buida
	  Item *pitem1 = first;
	  Item *pitem2 = q.first;
		  while (pitem1 != NULL and pitem2 != NULL){ // sumem els elements comuns
		  	pitem1->value += pitem2->value
		  	pitem1 = pitem1->next;
		  	pitem2 = pitem2->next;
		  }
		  if (pitem2 != NULL){
			  if (_size == 0){ // if (first == NULL)
			  	Item *pnou = new Item();
			  ...
			  while (pitem2 != NULL){
			  	...
			  	pnou->value = pitem2->value;
			  	pnou->next = NULL;
			  	last->next = pnou; // l'afegim al final
			  	last = pnou; // passa a ser l'últim
			  	_size++; // actualitzem la longitud
			  	pitem = pitem->value;
			}
		}
	}
